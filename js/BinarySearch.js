...
function init() {
	...
	var currentLinearSearch = new BinarySearch(animationManager, drawing.width, drawing.height) ;
}
...
BinarySearch.prototype.initArray = function( arrayNodeValue ) {
	...
	if( i < this.maxSize ){		
		this.arrayList[i] = new BinarySearchNode(this.objectID, this.nodeValue , parseInt(this.startX+this.head*(this.width-1)), this.startArrayY) ;
		// 创建矩形
		{
...
BinarySearch.prototype.initNumber = function ( number ){
	this.number = number ;
	this.numNode = new BinarySearchNode ( this.objectID ,this.number , this.startX , this.startY);
	    // 创建矩形
		{
